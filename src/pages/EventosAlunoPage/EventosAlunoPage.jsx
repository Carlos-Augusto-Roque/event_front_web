...
const EventosAlunoPage = () => {
  ...
  const loadMyCommentary = async (idUsuario, idEvento) => {
    ...
    try {
      ...
      const promise = await api.get(
        ...
        const comentario = promise.data
      setComentario(comentario.descricao);
      setIdComentario(comentario.idComentarioEvento);
      setExibeComentario(comentario.exibe);//pode vir true ou false

      // console.log(promise);
      // console.log(comentario);
      // console.log("Exibe? " + comentario.exibe);
  ...
  const postMyCommentary = async (descricao, idUsuario, idEvento) => {
    try {
      const promise = await api.post(commentaryEventResource, {
        descricao: descricao,
        exibe: true,//lá na api que é feita a validação e alterado para false, se necessário
        idUsuario: idUsuario,
        idEvento: idEvento,
      });
  ...
  const commentaryRemove = async (idComentario) => {
    ...
    if (window.confirm("Remover Comentário?") === false) {
      return false;
    }
    ...
    <>
      ...
      {showModal ? (
        <Modal
          ...
          exibeComentario={exibeComentario}
        />
      ) : null}
    </>
  );
